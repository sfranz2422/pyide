...
window.PyIDERuntime = (function () {
  ...
  var BOOTSTRAP = [
    ...
    "    A game loop is an infinite loop on purpose, and here it is not even",
    "    Python's loop — Kaplay drives it, so this function returns within",
    "    milliseconds while the game carries on. Nothing to time out, and no",
    "    per-line tracing to slow every callback down for the rest of the run.",
    "    \"\"\"",
    "    os.makedirs(PROJECT_DIR, exist_ok=True)",
    "    os.chdir(PROJECT_DIR)",
    "    _forget_project_modules()",
